...
def run_pipeline():
    ...
    try:
        ...
        if result:
            ...
            _log("  HeyGen failed — trying stock video fallback (Pexels)...")
            if avatar_generator.LAST_FAILURE_REASON == "INSUFFICIENT_CREDIT":
                _notify(
                    f"\u26a0\ufe0f <b>{config.CHANNEL_NAME}</b> \u2014 HeyGen credits exhausted\n"
                    f"Videos falling back to Pexels stock footage until topped up.\n"
                    f"Top up at: app.heygen.com"
                )
            result = stock_video_fallback.generate_stock_video(
                title=package["title"],
                script=package.get("script", ""),
                scenes=package.get("scenes", []),
                output_path=video_path,
            ...
            if result:
                _log("  Stock video fallback ready")
            else:
                _log("  Stock video failed — trying Leonardo slideshow as last resort...")
                ...
                if result:
                    _log("  Leonardo slideshow ready (last resort)")
                else:
                    raise RuntimeError("All video generation methods failed (HeyGen + Pexels + Leonardo)")
        ...
        if avatar_generator.LAST_FAILURE_REASON == "":
            source_tag = f"HeyGen ({config.HEYGEN_AVATAR_ID})"
        elif os.path.exists(video_path) and "stock" in result:
            source_tag = "Pexels stock footage"
        else:
            source_tag = "Leonardo slideshow"
        _notify(
        ...
        if "invalid_grant" in exc_str:
            _notify(
                f"\U0001f512 <b>{config.CHANNEL_NAME}</b> \u2014 YouTube token expired\n"
                f"Re-authorization required. Contact your admin to run the OAuth flow.\n"
                f"<i>No videos will upload until this is fixed.</i>"
            )
        else:
            _notify(
